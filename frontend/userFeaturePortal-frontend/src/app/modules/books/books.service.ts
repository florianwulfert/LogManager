...
export class BooksService implements OnDestroy{
  ...
  deleteBook(title: string | undefined): Observable<DeleteBookResponse> {
    this.actorFacade.stateActor$.pipe(takeUntil(this.onDestroy)).subscribe(r => {
      this.name = r
    })
    return this.http.delete<DeleteBookResponse>(API_DELETE_BOOK + title + '&actor=' + this.name, {
      observe: 'response'
    }).pipe(
      map((r) => {
        this.featureManager.openSnackbar("Book with the title " + title + " was deleted.");
        return r.body || {
          result: [],
          returnMessage: ""
        }
