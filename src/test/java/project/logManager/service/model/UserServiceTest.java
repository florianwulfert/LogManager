...
class UserServiceTest {
    ...
    @Test
    void testAddUser() {
        Mockito.when(bmiService.calculateBmiAndGetBmiMessage(Mockito.any(), Mockito.any(), Mockito.any())).thenReturn("User has a BMI of 24.07 and therewith he has normal weight.");
        Mockito.when(userValidationService.checkIfUsersListIsEmpty(Mockito.any(), Mockito.any(), Mockito.anyBoolean())).thenReturn(false);
        Mockito.when(userValidationService.checkIfNameExists(Mockito.anyString(), Mockito.anyBoolean())).thenReturn(users.get(1));
    ...
    void testUsersListIsEmpty() {
        Mockito.when(bmiService.calculateBmiAndGetBmiMessage(Mockito.any(), Mockito.any(),
                Mockito.any())).thenReturn("User has a BMI of 24.07 and therewith he has normal weight.");
        Mockito.when(userValidationService.checkIfUsersListIsEmpty(Mockito.anyString(), Mockito.any(), Mockito.anyBoolean())).thenReturn(true);
    ...
    void testDeleteById() {
        Mockito.when(userValidationService.checkIfNameExists(Mockito.anyString(),Mockito.anyBoolean())).thenReturn(users.get(1));
        Assertions.assertEquals(String.format(InfoMessages.USER_DELETED_ID, 1 ),
                systemUnderTest.deleteById(1, users.get(0).getName()));
        Mockito.verify(logService).addLog(String.format(InfoMessages.USER_DELETED_ID, 1), "WARNING", "Peter");
    }

    @Test
    void testDeleteByName() {
        Mockito.when(userValidationService.checkIfNameExists(Mockito.anyString(), Mockito.anyBoolean())).thenReturn(users.get(1));
        Assertions.assertEquals(String.format(InfoMessages.USER_DELETED_NAME, "Florian"),
                systemUnderTest.deleteByName("Florian", "Peter"));
        Mockito.verify(logService).addLog(String.format(InfoMessages.USER_DELETED_NAME, "Florian"), "WARNING", "Peter");
    }
